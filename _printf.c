 ...
 *         or -1 on error (e.g., trailing single '%').
 */
int handle_format(const char *format, int *i, va_list ap)
{
        ...
        (*i)++;
        /* If '%' is last character, that's an error: return -1 and print nothing */
        if (format[*i] == '\0')
        {
                return (-1);
        }
 ...
 *
 * Return: number of characters printed, or -1 on error / if format is NULL.
 */
